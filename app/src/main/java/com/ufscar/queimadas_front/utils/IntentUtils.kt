...
inline fun <reified T : Serializable> Intent.getSerializableExtra() =
    getSerializableExtra(T::class.java.name) as T?

fun <T : Parcelable> Intent.putParcelableExtra(value: T) = putExtra(value.javaClass.name, value)
fun <T : Serializable> Intent.putSerializableExtra(value: T) = putExtra(value.javaClass.name, value)
